...
_DEFAULT_MODULE_CANDIDATES = ("autoclanker_adapter", "cevolve_autoclanker_adapter")


class CevolveAdapter:
    kind = "cevolve"
    ...
    def _resolve_real_delegate(self) -> tuple[EvalLoopAdapter, str, str]:
        if self.config.mode == "auto":
                ...
                if path.exists():
                    return (
                        load_repo_adapter(
                            ...
                            detail=f"Resolved local cevolve repo at {path}",
                        ),
                        "local_repo_path",
                        f"Resolved local cevolve repo at {path}",
                    )
                resolution_errors.append(f"cevolve repo path not found: {path}")
            if self.config.python_module is not None:
                ...
                resolution_errors.append(
                    f"cevolve module not importable: {self.config.python_module}"
                )
            if self.config.command:
                ...
                resolution_errors.append(f"cevolve command not found: {command}")
            if resolution_errors:
                raise AdapterFailure("; ".join(resolution_errors))
            raise AdapterFailure(
                "cevolve auto mode did not receive any usable integration hint. Provide repo_path, python_module, or command."
            )
        if self.config.mode == "local_repo_path":
            path = resolved_repo_path(self.config)
            if not path.exists():
                raise AdapterFailure(f"cevolve repo path not found: {path}")
            return (
                load_repo_adapter(
                    self.config,
                    kind=self.kind,
        ...
        if self.config.mode == "installed_module":
            if self.config.python_module is None:
                raise AdapterFailure(
                    "cevolve installed_module mode requires python_module."
                )
        ...
        if self.config.mode == "subprocess_cli":
            ...
            if shutil.which(command) is None:
                raise AdapterFailure(f"cevolve command not found: {command}")
            return (
                JsonSubprocessAdapter(self.config, kind=self.kind),
                self.config.mode,
                f"Resolved subprocess command {command}",
            )
        raise AdapterFailure(f"Unsupported cevolve mode: {self.config.mode}")
