...
| `M4-008` | `required` | The goalloop adapter kind treats a goal loop's charter gates as the locked benchmark surface: candidate genotypes reach gates as GOALLOOP_GENE_* environment variables, gate exit codes and optional GOALLOOP_METRICS output become eval results carrying the loop's contract digest, and evaluation is refused when the loop contract has drifted from its lock. |
...
| `M8-002` | `required` | The issue-seed generator turns evidence, benchmark, and rough-lane seed input into a portable issue body, autoclanker ideas, run contract, lane ledger, kickoff prompts, executable headless command, deterministic default canonicalization, and optional host-adapter contract without requiring hosted services or provider secrets. |
| `M9-001` | `required` | The standalone clankerprof package decodes raw and gzipped pprof CPU profiles into a typed, language-neutral call graph model without requiring generated protobuf code at runtime. |
| `M9-002` | `required` | clankerprof preserves target-function attribution semantics by assigning target-contained sample self time to configured categories with complete Other catch-all attribution. |
| `M9-003` | `required` | clankerprof supports Ruby runtime rule packs for core-class/native semantic labeling, simplified versus verbose categories, runtime-internal folding, semantic caller tracking, and proportional CPU attributables without hardcoding Ruby as the core engine. |
| `M9-004` | `required` | As secondary migration coverage, clankerprof covers profile slicing with slice path attribution, generic declarative metadata, filters, collapse rules, attribution overrides, JSON slice output, and structured profile comparison without defining target-attribution semantics. |
| `M9-005` | `required` | clankerprof is usable both as a standalone CLI and through the autoclanker pprof alias, with machine-readable JSON results, file output for report formats, and regression comparison for slice or boundary reports. |
| `M9-006` | `required` | clankerprof exposes a stable versioned sample-facts JSON contract and projection-neutral fact index so target, boundary, slice, semantic-caller, and compare projections can share one decoded profile surface. |
| `M9-007` | `required` | clankerprof supports declarative scope decomposition with cost kinds, scope rollups, owner breakdowns, residual exclusions, calibrated attributables, expression selectors, configured cost-kind selectors, runtime-label selectors, legacy boundary/category/domain aliases, and cached frame-selector matching. |
| `M10-001` | `required` | goalloop provides a deterministic, host-neutral goal-loop contract: file-backed charter, tracker, and history state with wave-grouped requirement rows, a strict status vocabulary, charter-defined gates with real exit-code propagation, and a goal check that passes only when every row is finished and every gate succeeds. |
| `M10-002` | `required` | goalloop supports adversarial audit convergence: an auditor charter prompt requiring reproduction, triaged findings ingestion that appends confirmed findings as new tracker waves and refutations to the audit log, a bounded round budget that escalates to a human, and goal blocking until an audit round confirms nothing new. |
| `M10-003` | `required` | goalloop is usable standalone and through the autoclanker goalloop alias, emits harness-portable handoff prompts so any agent harness or human can continue a loop, and ships an operator skill documenting the Claude, Codex, and pi drivers. |
| `M10-004` | `required` | goalloop locks the charter's semantic contract (name, gates, audit policy) behind a recorded digest: prose edits stay free, but a drifted contract blocks the goal check until it is re-locked as an explicit, history-recorded act, mirroring the bayes-layer rule that eval results with a mismatched contract are rejected. |
