...
| Reserved projection row names fail closed: user slices may not use `(gc)`/`(uncollapsible)` and scope rollups may not use `Other` (the implicit leftover bucket) | covered | `test_clankerprof_reserved_pseudo_slice_names_rejected`, `test_clankerprof_scope_rollup_name_other_is_reserved`, `test_clankerprof_rust_round6_cluster_s_matches_python` |
...
| Python-dialect regexes (lookaround included) load and match in both languages within the pinned semantic subset; anchor/case-fold/Unicode-class edges are documented engine-native divergences pinned against drift; explicit invalid regexes fail closed with a shared message prefix | covered | `test_clankerprof_rust_regex_dialect_and_failclosed_match_python`, `test_clankerprof_regex_engine_edge_semantics_are_pinned`, `test_clankerprof_invalid_regex_patterns_fail_closed` |
