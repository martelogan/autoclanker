...
| Reserved projection row names fail closed: user slices AND `--attribute` targets (even virtual ones) may not use `(gc)`/`(uncollapsible)`, and scope rollups may not use `Other` (the implicit leftover bucket) | covered | `test_clankerprof_reserved_pseudo_slice_names_rejected`, `test_clankerprof_scope_rollup_name_other_is_reserved`, `test_clankerprof_rust_round6_cluster_s_matches_python`, `test_clankerprof_rust_round7_cluster_v_matches_python` |
