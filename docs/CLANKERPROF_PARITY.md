...
| Strict pprof decoding rejects illegal field number zero and invalid string-table UTF-8 with shared messages | covered | `test_clankerprof_decoder_rejects_field_zero_and_invalid_utf8`, `test_clankerprof_rust_round5_cluster_q_matches_python` |
| Facts v2 profile metadata is validated: `primary_value_index` required, string-domain fields reject non-strings (shared messages) | covered | `test_clankerprof_facts_profile_metadata_strictness`, `test_clankerprof_rust_round5_cluster_q_matches_python` |
| Recursion limits fail closed: JSON nesting mirrors serde_json's 128-level limit byte-identically; YAML cycles/deep nesting share the "recursion limit exceeded" core | covered | `test_clankerprof_recursion_limit_envelopes`, `test_clankerprof_rust_round5_cluster_q_matches_python` |
| Duplicate slice names are rejected in both languages (previously first-wins vs last-wins metadata) | covered | `test_clankerprof_duplicate_slice_names_rejected`, `test_clankerprof_rust_round5_cluster_q_matches_python` |
| Aggregation identity is structural: frame rows key on (function, filename), caller/leaf evidence on (caller, leaf); delimiter collisions render as distinct rows | covered | `test_clankerprof_aggregation_keys_use_tuple_identity`, `test_clankerprof_rust_round5_cluster_q_matches_python` |
| Bare CR in core-class CSV is a record break through the real loaders in both languages (verified non-divergence, pinned) | covered | `test_clankerprof_core_class_csv_bare_cr_is_a_record_break`, `test_clankerprof_rust_round5_cluster_q_matches_python` |
| A present facts `summary` must be a JSON object (null reads as absent); wrong-typed summaries are validation errors in both languages | covered | `test_clankerprof_facts_import_edge_branches`, `test_clankerprof_rust_r4_slices_scopes_facts_validation_matches_python` |
...
| Category precedence is first-match-wins in config order; ranked arrays break ties by first-seen order (Rust parity via insertion-ordered maps), including scope owner-file function truncation under `--top` | covered | `test_clankerprof_target_category_precedence_and_tie_order`, `test_clankerprof_rust_round5_cluster_p_matches_python` |
...
| Packaged Ruby core class CSV is used by default for `--runtime ruby`, with explicit override still available; quoted CSV fields parse with `csv.reader` semantics in both languages, including quoted fields spanning newlines | covered | `test_clankerprof_loads_packaged_ruby_core_classes_by_default`, `test_clankerprof_rust_target_renderer_semantics_match_python`, Rust `core_classes_csv_record_scanning_matches_python_csv_reader`, `test_clankerprof_rust_round5_cluster_p_matches_python` |
...
| Attributable estimates scale by signed share for any nonzero scope total (suppressed only at exactly zero) in both languages | covered | `test_clankerprof_scope_attributables_scale_for_negative_totals`, `test_clankerprof_rust_r4_slices_scopes_facts_validation_matches_python` |
...
| Zero-aggregate rollup rows are omitted while negative rows render, keeping bucket/owner sums equal to scope totals byte-identically; a row is omitted only when its aggregate AND its rendered subtree are zero (cancelled buckets keep their nonzero categories) | covered | `test_clankerprof_rust_numeric_edge_semantics_match_python`, `test_clankerprof_scope_rollups_render_negative_costs_additively`, `test_clankerprof_zero_sum_buckets_keep_nonzero_categories`, `test_clankerprof_rust_round6_cluster_s_matches_python` |
| Reserved projection row names fail closed: user slices AND `--attribute` targets (even virtual ones) may not use `(all)`/`(gc)`/`(uncollapsible)`, and scope rollups may not use `Other` (the implicit leftover bucket) | covered | `test_clankerprof_reserved_pseudo_slice_names_rejected`, `test_clankerprof_scope_rollup_name_other_is_reserved`, `test_clankerprof_rust_round6_cluster_s_matches_python`, `test_clankerprof_rust_round7_cluster_v_matches_python` |
| The numeric-backref rewrite understands `(?x)` extended-mode `#` line comments (comment parens never shift group numbering; scoped/mid-pattern x flags decline fail-closed) | covered | `numeric_backref_rewrite_understands_extended_mode_comments` (Rust), `test_clankerprof_rust_round9_cluster_y_matches_python` |
| Zero-sum categories/domains render when any emitted evidence map (leaf functions, folded-from, caller-to-leaf pairs, per-file maps) is nonzero | covered | `test_clankerprof_zero_sum_rollups_keep_caller_pair_evidence`, `test_clankerprof_rust_round9_cluster_y_matches_python` |
| Core-class explicit-native routing is exclusion-aware and keys on markers/pseudo-paths only (`native_path_patterns` feed general native detection, never core routing) | covered | `test_clankerprof_core_class_native_routing_is_exclusion_aware`, `test_clankerprof_rust_round9_cluster_y_matches_python` |
| v2 `sample_index` must equal the sample's zero-based array position (duplicates/gaps/non-zero starts rejected identically; v1 leniency preserved) | covered | `test_clankerprof_facts_v2_sample_index_must_match_position`, `test_clankerprof_rust_round9_cluster_y_matches_python` |
| Slice percentages are shares of the filtered matching total with a zero arm — never a fallback to the whole-profile total when signed matches cancel | covered | `test_clankerprof_zero_sum_filtered_slices_use_zero_percentages`, `test_clankerprof_rust_round6_cluster_s_matches_python` |
...
| Non-finite slice metadata numbers fail closed with one shared message in both languages (never a silent null) | covered | `test_clankerprof_slice_metadata_rejects_non_finite_numbers`, `test_clankerprof_rust_r4_slices_scopes_facts_validation_matches_python` |
...
| Attribution overrides, including invalid/duplicate filter rejection and empty predicate keys/values (`name:` would match every frame) | covered | `test_clankerprof_slice_cli_validates_attribute_contract`, `test_clankerprof_rust_round7_cluster_v_matches_python` |
...
| Negated descendant filters (`<!pred`) bind to descendant existence: stacks containing the forbidden frame are excluded in both languages | covered | `test_clankerprof_slice_descendant_filters_use_or_semantics`, `test_clankerprof_rust_r4_slices_scopes_facts_validation_matches_python` |
| Repeated `!` and `<` filter prefixes in any order | covered | `test_clankerprof_slice_filter_prefixes_can_repeat_in_any_order` |
| Bottom `slice:<name>` filters include descendant-attribute rescue behavior, resolving the first-match winning rule (a losing rule targeting the slice never matches) | covered | `test_clankerprof_slice_filter_honors_descendant_attribute_slice_matches`, `test_clankerprof_slice_filter_rescue_uses_winning_attribute_rule`, `test_clankerprof_rust_round8_cluster_w_matches_python`, Rust `bottom_slice_filters_honor_descendant_attribute_rescue` |
| Negated `!slice:<name>` filters exclude descendant-attributed samples (rescue applies to both polarities) | covered | `test_clankerprof_slice_filter_negation_respects_descendant_attribution`, `test_clankerprof_rust_round5_cluster_p_matches_python` |
...
| Zero-sum categories with nonzero signed leaf subtrees render at every rollup level (bucket, domain, and category rows) | covered | `test_clankerprof_scope_zero_sum_categories_render_with_signed_children`, `test_clankerprof_rust_round8_cluster_w_matches_python` |
| Scope `slice:<name>` predicates test effective first-match ownership (overlapping later definitions own nothing) | covered | `test_clankerprof_scope_slice_predicates_use_effective_ownership`, `test_clankerprof_rust_round8_cluster_w_matches_python` |
| A present facts summary requires all four members; `summary: {}` or null members are validation errors | covered | `test_clankerprof_facts_summary_requires_all_members`, `test_clankerprof_rust_round8_cluster_w_matches_python` |
| Slice files loaded through scope configs run the shared definition validation (duplicates, multiple defaults, reserved names) | covered | `test_clankerprof_scope_config_validates_loaded_slice_definitions`, `test_clankerprof_rust_round8_cluster_w_matches_python` |
| `primary_value_index` must equal the documented selection from `default_sample_type`/`value_types` | covered | `test_clankerprof_facts_primary_index_must_match_selection`, `test_clankerprof_rust_round8_cluster_w_matches_python` |
...
| `--by-slice` fails closed on unparsable values and non-finite thresholds, including integral config floats beyond int64 (never clamped); negative limits drop slices from the tail, identically across languages | covered | `test_clankerprof_by_slice_values_validate_and_support_negative_limits`, `test_clankerprof_rust_slices_validation_envelopes_match_python`, `test_clankerprof_rust_cli_flag_matrix_matches_python`, `test_clankerprof_rust_round5_cluster_p_matches_python` |
...
| Python-dialect regexes (lookaround included) load and match in both languages within the pinned semantic subset; numeric backrefs to named groups compile via rewrite; anchor/case-fold/Unicode-class/conditional-group edges are documented engine-native divergences pinned against drift; explicit invalid regexes fail closed with a shared message prefix | covered | `test_clankerprof_rust_regex_dialect_and_failclosed_match_python`, `test_clankerprof_regex_engine_edge_semantics_are_pinned`, `test_clankerprof_invalid_regex_patterns_fail_closed`, `test_clankerprof_rust_round7_cluster_v_matches_python` |
...
| Multi-member (concatenated) gzip profiles decode fully in both languages; zero padding after or between members is consumed (gzip FAQ #8), and non-zero trailing bytes fall back to raw-protobuf parsing identically | covered | `test_clankerprof_rust_cli_flag_matrix_matches_python` (facts-multimember-gzip), Rust `concatenated_gzip_members_all_decode`, `zero_padded_gzip_members_decode`, `test_clankerprof_rust_round7_cluster_v_matches_python` |
...
| Slices files require the top-level `slices` array (missing key or null = the shared validation error, never a silently empty config) | covered | `test_clankerprof_slices_file_requires_top_level_slices_key`, `test_clankerprof_rust_round8_cluster_x_matches_python` |
| Mixed global/local `--output` keeps the last occurrence in typed order (hoist inserts after the subcommand); global-only placement still writes | covered | Rust `hoist_keeps_a_later_local_output_winning`, `test_clankerprof_rust_round8_cluster_x_matches_python` |
| Deprecated protobuf group fields (wire types 3/4) skip when balanced and error identically when mismatched, unmatched, truncated, or over-nested | covered | `test_clankerprof_decoder_skips_balanced_unknown_groups`, Rust `group_skip_tests`, `test_clankerprof_rust_round8_cluster_x_matches_python` |
| The numeric-backref rewrite is `(?#…)`-comment-aware: comment parentheses never shift group numbering and comment bodies are never rewritten | covered | Rust `regex_comment_tests`, `test_clankerprof_rust_round8_cluster_x_matches_python` |
| Explicitly empty path option values (`--output ''` et al.) are rejected in both languages (engine-specific message prose) | covered | `test_clankerprof_empty_path_option_values_are_rejected`, `test_clankerprof_rust_round8_cluster_x_matches_python` |
| `--show-paths` is the documented compatibility no-op: identical bytes with and without the flag, in and across both languages | covered | `test_clankerprof_rust_round8_cluster_x_matches_python` |
| The comma is reserved by the focus grammar: comma-bearing slice names and scope labels are rejected at validation, and focus against pre-existing comma-named reports fails closed with the shared envelope | covered | `test_clankerprof_comma_names_rejected_and_focus_fails_closed`, `test_clankerprof_rust_round10_cluster_z_matches_python` |
| The slice-options config is fixed-shape: unknown top-level keys exit 2 with `Unknown slice config key: <key>.` in both languages | covered | `test_clankerprof_slice_config_rejects_unknown_keys`, `test_clankerprof_rust_round10_cluster_z_matches_python` |
| Rust `report` sections apply their standalone subcommands' validation: empty target config errors like `targets`, and `--top` is validated unconditionally | covered | Rust `report_validation_tests`, `test_clankerprof_rust_round10_cluster_z_matches_python` |
| Protobuf field numbers above `2^29 - 1` are decode errors with the shared `Illegal protobuf field number <n>.` envelope; the maximum legal field still skips | covered | `test_clankerprof_decoder_rejects_over_maximum_field_numbers`, `test_clankerprof_rust_round10_cluster_z_matches_python` |
| Top-level scope config keys are fixed-shape; a misspelled section (`owenr:`) fails closed with the shared message | covered | `test_clankerprof_scope_config_rejects_unknown_top_level_keys`, `test_clankerprof_rust_round8_cluster_x_matches_python` |
| Repeated singular `period_type` protobuf fields merge per protobuf semantics instead of last-occurrence replacement | covered | `test_clankerprof_decoder_merges_repeated_period_type_fields`, Rust `group_skip_tests`, `test_clankerprof_rust_round8_cluster_x_matches_python` |
...
| Unknown focus names fail closed against the union of before/after row names (identical envelopes; added/removed rows stay focusable) | covered | `test_clankerprof_compare_unknown_focus_names_fail_closed`, `test_clankerprof_rust_round6_cluster_u_matches_python` |
| Repeating any non-accumulating option (scalar or boolean) keeps the last occurrence in both CLIs; only `--target`/`--filter`/`--collapse`/`--attribute` accumulate | covered | `test_clankerprof_repeated_scalar_options_keep_last_occurrence`, `test_clankerprof_rust_round6_cluster_u_matches_python` |
| Elapsed-time formatting shares the f64-operand division order above `2^53` (csv and text byte-identical) | covered | `test_clankerprof_rust_round6_cluster_u_matches_python` |
| CPython-repr float spelling includes half-to-even midpoint ties (flt2dec's away-from-zero tie repaired exactly) | covered | pyjson `pyfloat_matches_cpython_repr_grammar` tie rows, `test_clankerprof_rust_round6_cluster_u_matches_python` |
...
| Explicit YAML tags follow serde_yaml semantics: typed core tags apply the strict scalar grammars, other global tags resolve to the underlying node (deterministic — no constructed native objects), local `!name` tags fail closed with one shared message on every YAML surface | covered | `test_clankerprof_yaml_explicit_tags_follow_serde_semantics`, `test_clankerprof_rust_yaml_tags_match_python` |
| YAML merge keys (`<<`, plain or quoted, incl. explicit `!!merge`) fail closed with one shared message on every YAML surface | covered | `test_clankerprof_yaml_merge_keys_rejected`, `test_clankerprof_rust_round6_cluster_t_matches_python` |
| Unpaired UTF-16 surrogate escapes are validation errors on every JSON and YAML surface (serde message cores; JSON pairs compose astral chars, YAML pairs never combine and reject) | covered | `test_clankerprof_json_and_yaml_inputs_reject_lone_surrogates`, `test_clankerprof_rust_round6_cluster_t_matches_python` |
| Core-class CSV fields of any length parse identically (no 131072-byte csv-module cap; residual csv errors contracted to the envelope) | covered | `test_clankerprof_core_class_csv_accepts_large_fields`, `test_clankerprof_rust_round6_cluster_t_matches_python` |
| Scope config `slices` must be a string path; non-strings fail closed with one shared message (never Rust's silent-absent or Python's coerced path) | covered | `test_clankerprof_scope_config_slices_must_be_string`, `test_clankerprof_rust_round6_cluster_t_matches_python` |
...
| Zero-total simple-csv keeps every nonzero signed row (only exactly-zero rows may be omitted), and `--by-slice` %-thresholds compare against the rendered zero percentages at a zero matching total | covered | `test_clankerprof_zero_sum_filtered_slices_use_zero_percentages`, `test_clankerprof_zero_total_simple_csv_keeps_signed_rows`, `test_clankerprof_rust_round7_cluster_v_matches_python` |
| Scope/boundary entries reject unknown keys (typos never silently disable a rule) with a shared message | covered | `test_clankerprof_scope_entries_reject_unknown_keys`, `test_clankerprof_rust_round7_cluster_v_matches_python` |
| Facts v2 rejects a `primary_value_index` outside the declared value types (no selection rule can produce one) | covered | `test_clankerprof_facts_rejects_out_of_range_primary_value_index`, `test_clankerprof_rust_round7_cluster_v_matches_python` |
